...
**Idea:** highlighting *is* filtering that keeps every row. Same match logic as filter
...
- Reuse the existing match evaluation — only the *application* differs (hide vs mark).
...
**Not a new card component.** We are *not* designing a new card. It is the **exact JSON view we already
...
- **Filters *and* highlight apply in both view modes.** Highlight is first-class in JSON mode, not just
  the table: a highlight match tints the entry **and** emphasizes the matched key/value *inside* the
...
summary row — the chips/indicators that say *which* filters and highlights are active — and the log
...
- **Highlight granularity** — whole row, or just the matching field cell? *(lean: row tint + emphasize
  the matched cell.)*
- **Multiple highlight clauses** — different colors per clause, or one highlight color? *(lean: one to
  start.)*
...
- **Scope** — Documentation inline log table gets the same modes, or `/logs` first? *(lean: `/logs`
  first.)*
